...
> 目标：让电视大屏连接到一个**固定网址**，在该网址的**后台登录、上传数据/文件**，大屏**每 30 秒自动拉取更新**。
> 整套方案**只依赖你的 GitHub 账号**，无需任何服务器、无需付费。

> **本项目固定域名：** `https://dawndsre.github.io/party-screen/`
> 改造细节（固定域名、脱敏规则、更新机制）见 **[`改造说明.md`](./改造说明.md)**。
...
- **固定网址** = `https://dawndsre.github.io/party-screen/`（GitHub Pages 免费提供）
- **后台登录** = 打开 `admin.html`，填入你的 GitHub 令牌（PAT），即可编辑/上传
- **电视更新** = 打开上面的固定网址即可，大屏每 30 秒自动联网拉取
...
| 文件 | 作用 |
|------|------|
| `index.html` | 电视大屏页面，每 30 秒拉取 `screen-data.json`（相对路径，自动指向固定域名） |
| `admin.html` | 后台管理，登录后编辑/上传，写入仓库；发布时自动脱敏 |
| `screen-data.json` | 电视拉取的脱敏展示数据（发布后生成） |
| `data/party.json` | 后台编辑的完整源数据（会议文本同样已脱敏） |
| `.github/workflows/pages.yml` | GitHub Pages 自动部署（Auto Deploy） |
| `启动大屏（Edge全屏）.bat` | 一键用 Edge 全屏模式打开展示页 |
| `启动后台.bat` | 一键打开后台管理页 |
| `改造说明.md` | 固定域名 + 脱敏规则 + 更新机制说明 |
